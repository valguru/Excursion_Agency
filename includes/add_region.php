...
if (isset($_POST['add-region'])) {
    ...
    if(!isset($_SESSION['error_upload_avatar']) && !isset($_SESSION['wrong_avatar_extension'])) {
        mysqli_query($connect, "CALL insert_region('$name', '$path', '$description')") or die(mysqli_error($connect));
        $_SESSION['message_successful_signup'] = "<p>Регион успешно добавлен!</p>";
        header("Location: ../pages/regions.php");
    }
}
